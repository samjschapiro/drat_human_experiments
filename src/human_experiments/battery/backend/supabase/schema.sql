...
-- Pre-seed N slots. The upper bound MUST equal TOTAL_SLOTS - 1, where
-- TOTAL_SLOTS is printed by prepare_battery.py (= n_orders * n_anchor_sets *
-- replications_per_cell). The example battery config prints TOTAL_SLOTS=160.
-- A "slot" here is just an atomic balanced counter; the frontend decodes it
-- into (counterbalance order, DRAT anchor set).
insert into slots (id)
    select gs from generate_series(0, 159) gs   -- 160 slots (0..159); match TOTAL_SLOTS
