...
# Write the runtime config (gitignored). core.js reads window.BATTERY_RUNTIME;
# we never mutate the tracked core.js, so redeploys stay clean.
echo ""
echo "Writing js/config.js (API_BASE=$API_BASE)"
cat > js/config.js <<EOF
// Auto-generated by deploy.sh — do not edit by hand. Gitignored.
